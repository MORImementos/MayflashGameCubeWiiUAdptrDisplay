...
function pollControllers()
{
    ...
    if(controllerIndex === -1 && navigator.getGamepads().length > 0)
    {
        ...
        for(n = 0; n < gamepadArray.length; n++)
        {
			...
			if (gamepadArray[n] && gamepadArray[n].id.toLowerCase().includes("mayflash") && (Math.abs(gamepadArray[n].axes[0]) > 0.51 || Math.abs(gamepadArray[n].axes[1]) > 0.51 || gamepadArray[n].buttons[1].pressed || gamepadArray[n].buttons[9].pressed))
            {
				controllerIndex = n;
    ...
    {
		selectedPad = navigator.getGamepads()[controllerIndex];
